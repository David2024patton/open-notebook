...
3. First user to register becomes superuser (auto-promoted when no users exist yet)
...
- Users table is isolated — existing notebooks/sources are auto-assigned to the first superuser during registration. Admins can also claim unowned notebooks via `POST /api/auth/claim-legacy-content`.
