...
async def content_process(state: SourceState) -> dict:
    ...
    # Git repository URL detection (modular add-on): if the URL is a Git repo,
    # extract its content via the git helper before falling through to
    # content-core. Gated on the URL being present and matching is_git_url().
    ...
    if url and is_git_url(url):
        ...
        try:
            ...
            raise ValueError(f"Failed to process Git repository: {str(e)}")
