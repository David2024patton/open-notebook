...
# Rewrite the SSH user/password from env vars if set (multi-user SSH setup).
if [ -x /app/scripts/setup-ssh.sh ]; then
    /app/scripts/setup-ssh.sh || log "WARNING: SSH setup failed; continuing with defaults."
fi
