...
# Add git for repository cloning support, openssh-server for SSH access, and
# the Playwright/Chromium runtime libraries for the browser-agent (Phase 6).
RUN apt-get update && apt-get upgrade -y && apt-get install -y --no-install-recommends \
    ffmpeg \
    supervisor \
    curl \
...
# Install Playwright's Chromium browser for the browser-agent (Phase 6) and
# for the optional Crawl4AI local runtime. PLAYWRIGHT_BROWSERS_PATH below keeps
# the browser under the persisted data cache so it downloads once per volume.
